...
const FeatureCard = memo(function FeatureCard({ f }) {
  ...
  return (
    <div className={`bg-white border border-border-surface rounded-2xl p-6 h-full
      ...
      <h3 className="text-[17px] font-bold text-heading mb-2"
      ...
      {f.notification && (
        <div className="mt-4 space-y-2">
          {f.notification.map((n) => (
            <div key={n.label} className={`bg-bg-features border-l-4 ${n.border} rounded-r-lg px-3 py-2`}>
              <p className="text-[13px] font-semibold text-heading">{n.label}</p>
